...
switch ($filter) {
	case "owned":
		...
		break;
	case "shared":
		$q = "SELECT folder_sharing.id, folders.folder_name, folders.id AS afid FROM folder_sharing JOIN folders ON folders.id = folder_sharing.folder_id WHERE folder_sharing.receiver_id = {$_SESSION['id']} AND parent_id = $parentid";
		break;
	case "all":
